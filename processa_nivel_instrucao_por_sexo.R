# Distribuicao percentual ponderada do nivel de instrucao por sexo - Brasil
# Processa todos os recortes PNADc disponiveis em "saida/pnadc_*_t*_recorte.rds"

dir_entrada <- "saida"
arquivo_saida_distribuicao <- file.path(
  "saida",
  "distribuicao_nivel_instrucao_por_sexo_ano_trimestre.csv"
)
arquivo_saida_json <- file.path(
  "saida",
  "distribuicao_nivel_instrucao_por_sexo_ano_trimestre.json"
)
...
for (i in seq_along(arquivos_entrada)) {
  ...
  sexo <- trimws(as.character(dados[[codigo_variavel_dimensao]]))
  sexo[sexo %in% c("", "NA", "N/A", "NULL")] <- NA_character_
  ...
  sexo_invalido <- is.na(sexo)
  valido <- !(periodo_invalido | peso_invalido | nivel_invalido | sexo_invalido)

  n_total <- nrow(dados)
  n_validos <- sum(valido)
  n_excluidos <- n_total - n_validos
  n_sem_periodo_valido <- sum(periodo_invalido)
  n_sem_sexo <- sum(sexo_invalido)
  ...
  if (n_validos > 0) {
    ...
    agg_nivel <- aggregate(
      peso ~ Ano + Trimestre + sexo_valor + nivel_instrucao_valor,
      data = dados_validos,
      FUN = sum
    )
  ...
  idx_dimensao <- !periodo_invalido & !sexo_invalido
  if (sum(idx_dimensao) > 0) {
    dados_dimensao <- data.frame(
      Ano = ano[idx_dimensao],
      Trimestre = trimestre[idx_dimensao],
      sexo_valor = sexo[idx_dimensao],
      valido = as.integer(valido[idx_dimensao]),
      one = 1L,
      stringsAsFactors = FALSE
    )

    total_dimensao <- aggregate(one ~ Ano + Trimestre + sexo_valor, data = dados_dimensao, FUN = sum)
    validos_dimensao <- aggregate(valido ~ Ano + Trimestre + sexo_valor, data = dados_dimensao, FUN = sum)
    contagem_dimensao <- merge(
      total_dimensao,
      validos_dimensao,
      by = c("Ano", "Trimestre", "sexo_valor"),
      all = TRUE
    )
    names(contagem_dimensao) <- c("Ano", "Trimestre", "sexo_valor", "registros_total", "registros_validos")
...
if (length(lista_agregados_nivel) == 0) {
  stop("Nao ha registros validos para calcular distribuicao ponderada por sexo.")
}
...
names(resultado)[names(resultado) == "sexo_valor"] <- nome_campo_dimensao
...
if (length(lista_agregados_contagem) > 0) {
  ...
  contagem_dimensao <- aggregate(
    cbind(registros_total, registros_validos) ~ Ano + Trimestre + sexo_valor,
...
names(contagem_dimensao)[names(contagem_dimensao) == "sexo_valor"] <- nome_campo_dimensao

resumo_dimensao <- merge(
  contagem_dimensao,
  {
    total_tmp <- total_ponderado_dimensao
    names(total_tmp)[names(total_tmp) == "sexo_valor"] <- nome_campo_dimensao
    total_tmp
  },
...
series_por_sexo <- lapply(names(split_dimensao), function(valor_dimensao) {
