# Distribuicao percentual ponderada do nivel de instrucao por regiao - Brasil
# Processa todos os recortes PNADc disponiveis em "saida/pnadc_*_t*_recorte.rds"

dir_entrada <- "saida"
arquivo_saida_distribuicao <- file.path(
  "saida",
  "distribuicao_nivel_instrucao_por_regiao_ano_trimestre.csv"
)
arquivo_saida_json <- file.path(
  "saida",
  "distribuicao_nivel_instrucao_por_regiao_ano_trimestre.json"
)
...
codigo_variavel_regiao <- "Regiao_UF"
codigo_variavel_uf <- "UF"
nome_campo_dimensao <- "regiao"
...
derivar_regiao_por_uf <- function(uf) {
...
for (i in seq_along(arquivos_entrada)) {
  ...
  regiao <- if (codigo_variavel_regiao %in% names(dados)) {
    trimws(as.character(dados[[codigo_variavel_regiao]]))
  } else {
    rep(NA_character_, nrow(dados))
  }

  regiao[regiao %in% c("", "NA", "N/A", "NULL")] <- NA_character_

  if (any(is.na(regiao)) && codigo_variavel_uf %in% names(dados)) {
    regiao_fallback <- derivar_regiao_por_uf(dados[[codigo_variavel_uf]])
    idx_na <- is.na(regiao)
    regiao[idx_na] <- regiao_fallback[idx_na]
  }
  ...
  regiao_invalida <- is.na(regiao)
  valido <- !(periodo_invalido | peso_invalido | nivel_invalido | regiao_invalida)

  n_total <- nrow(dados)
  n_validos <- sum(valido)
  n_excluidos <- n_total - n_validos
  n_sem_periodo_valido <- sum(periodo_invalido)
  n_sem_regiao <- sum(regiao_invalida)
  ...
  resumo_arquivos <- rbind(
    ...
    data.frame(
      ...
      registros_sem_regiao = n_sem_regiao,
      stringsAsFactors = FALSE
    )
  )
  ...
  if (n_validos > 0) {
    ...
    agg_nivel <- aggregate(
      peso ~ Ano + Trimestre + regiao_valor + nivel_instrucao_valor,
      data = dados_validos,
      FUN = sum
    )
  ...
  idx_dimensao <- !periodo_invalido & !regiao_invalida
  if (sum(idx_dimensao) > 0) {
    dados_dimensao <- data.frame(
      Ano = ano[idx_dimensao],
      Trimestre = trimestre[idx_dimensao],
      regiao_valor = regiao[idx_dimensao],
      valido = as.integer(valido[idx_dimensao]),
      one = 1L,
      stringsAsFactors = FALSE
    )

    total_dimensao <- aggregate(one ~ Ano + Trimestre + regiao_valor, data = dados_dimensao, FUN = sum)
    validos_dimensao <- aggregate(valido ~ Ano + Trimestre + regiao_valor, data = dados_dimensao, FUN = sum)
    contagem_dimensao <- merge(
      total_dimensao,
      validos_dimensao,
      by = c("Ano", "Trimestre", "regiao_valor"),
      all = TRUE
    )
    names(contagem_dimensao) <- c("Ano", "Trimestre", "regiao_valor", "registros_total", "registros_validos")
...
if (length(lista_agregados_nivel) == 0) {
  stop("Nao ha registros validos para calcular distribuicao ponderada por regiao.")
}
...
total_ponderado_dimensao <- aggregate(
  peso ~ Ano + Trimestre + regiao_valor,
  data = agregado_nivel,
  FUN = sum
)
...
names(resultado)[names(resultado) == "regiao_valor"] <- nome_campo_dimensao
...
if (length(lista_agregados_contagem) > 0) {
  ...
  contagem_dimensao <- aggregate(
    cbind(registros_total, registros_validos) ~ Ano + Trimestre + regiao_valor,
...
names(contagem_dimensao)[names(contagem_dimensao) == "regiao_valor"] <- nome_campo_dimensao

resumo_dimensao <- merge(
  contagem_dimensao,
  {
    total_tmp <- total_ponderado_dimensao
    names(total_tmp)[names(total_tmp) == "regiao_valor"] <- nome_campo_dimensao
    total_tmp
  },
...
series_por_regiao <- lapply(names(split_dimensao), function(valor_dimensao) {
...
json_saida <- list(
  metadata = list(
    ...
    variavel_dimensao = codigo_variavel_regiao,
    variavel_dimensao_fallback = codigo_variavel_uf,
    rotulo_dimensao = "Regiao",
    descricao = "Distribuicao percentual ponderada de nivel de instrucao por regiao, ano e trimestre."
  ),
  cortes = cortes_json,
  series_por_regiao = series_por_regiao,
