# Participacao percentual ponderada de pessoas com "Superior completo" por UF
# Processa todos os recortes PNADc disponiveis em "saida/pnadc_*_t*_recorte.rds"

dir_entrada <- "saida"
arquivo_saida_distribuicao <- file.path(
  "saida",
  "participacao_superior_completo_por_unidade_da_federacao_ano_trimestre.csv"
)
arquivo_saida_json <- file.path(
  "saida",
  "participacao_superior_completo_por_unidade_da_federacao_ano_trimestre.json"
)
...
if (length(lista_agregados_peso) == 0) {
  stop("Nao ha registros validos para calcular participacao de superior completo por UF.")
}
...
json_saida <- list(
  metadata = list(
    ...
    descricao = "Participacao percentual, por periodo, de pessoas com superior completo distribuidas por UF (com pesos)."
  ),
  cortes = cortes_json,
