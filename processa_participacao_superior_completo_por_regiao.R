# Participacao percentual ponderada de pessoas com "Superior completo" por regiao
# Processa todos os recortes PNADc disponiveis em "saida/pnadc_*_t*_recorte.rds"

dir_entrada <- "saida"
arquivo_saida_distribuicao <- file.path(
  "saida",
  "participacao_superior_completo_por_regiao_ano_trimestre.csv"
)
arquivo_saida_json <- file.path(
  "saida",
  "participacao_superior_completo_por_regiao_ano_trimestre.json"
)

codigo_variavel_nivel_instrucao <- "VD3004"
valor_referencia_nivel_instrucao <- "Superior completo"
codigo_variavel_peso <- "V1028"
codigo_variavel_dimensao <- "Regiao_UF"
codigo_variavel_dimensao_fallback <- "UF"
...
for (i in seq_along(arquivos_entrada)) {
  ...
  if (!(codigo_variavel_dimensao %in% names(dados)) && !(codigo_variavel_dimensao_fallback %in% names(dados))) {
    stop(
      "Sem coluna de regiao e sem fallback de UF em ",
      basename(arquivo),
      ". Esperado: ",
      codigo_variavel_dimensao,
      " ou ",
      codigo_variavel_dimensao_fallback
    )
  }
  ...
  if (codigo_variavel_dimensao %in% names(dados)) {
    dimensao <- trimws(as.character(dados[[codigo_variavel_dimensao]]))
  } else {
    dimensao <- rep(NA_character_, nrow(dados))
  }
  dimensao[dimensao %in% c("", "NA", "N/A", "NULL")] <- NA_character_

  if (any(is.na(dimensao)) && codigo_variavel_dimensao_fallback %in% names(dados)) {
    dimensao_fallback <- derivar_regiao_por_uf(dados[[codigo_variavel_dimensao_fallback]])
    idx_na <- is.na(dimensao)
    dimensao[idx_na] <- dimensao_fallback[idx_na]
  }
...
if (length(lista_agregados_peso) == 0) {
  stop("Nao ha registros validos para calcular participacao de superior completo por regiao.")
}
...
json_saida <- list(
  metadata = list(
    ...
    variavel_dimensao_fallback = codigo_variavel_dimensao_fallback,
    descricao = "Participacao percentual, por periodo, de pessoas com superior completo distribuidas por regiao (com pesos)."
  ),
  cortes = cortes_json,
