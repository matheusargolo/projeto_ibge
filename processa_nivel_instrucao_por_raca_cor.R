# Distribuicao percentual ponderada do nivel de instrucao por raca/cor - Brasil
# Processa todos os recortes PNADc disponiveis em "saida/pnadc_*_t*_recorte.rds"

dir_entrada <- "saida"
arquivo_saida_distribuicao <- file.path(
  "saida",
  "distribuicao_nivel_instrucao_por_raca_cor_ano_trimestre.csv"
)
arquivo_saida_json <- file.path(
  "saida",
  "distribuicao_nivel_instrucao_por_raca_cor_ano_trimestre.json"
)
...
codigo_variavel_dimensao <- "V2010"
nome_campo_dimensao <- "cor_ou_raca"
...
resumo_arquivos <- data.frame(
  ...
  registros_sem_raca_cor = integer(),
...
for (i in seq_along(arquivos_entrada)) {
  ...
  raca_cor <- trimws(as.character(dados[[codigo_variavel_dimensao]]))
  raca_cor[raca_cor %in% c("", "NA", "N/A", "NULL")] <- NA_character_
  ...
  raca_cor_invalido <- is.na(raca_cor)
  valido <- !(periodo_invalido | peso_invalido | nivel_invalido | raca_cor_invalido)

  n_total <- nrow(dados)
  n_validos <- sum(valido)
  n_excluidos <- n_total - n_validos
  n_sem_periodo_valido <- sum(periodo_invalido)
  n_sem_raca_cor <- sum(raca_cor_invalido)
  ...
  resumo_arquivos <- rbind(
    ...
    data.frame(
      ...
      registros_sem_raca_cor = n_sem_raca_cor,
      stringsAsFactors = FALSE
    )
  )
  ...
  if (n_validos > 0) {
    dados_validos <- data.frame(
      ...
      raca_cor_valor = raca_cor[valido],
    ...
    agg_nivel <- aggregate(
      peso ~ Ano + Trimestre + raca_cor_valor + nivel_instrucao_valor,
      data = dados_validos,
      FUN = sum
    )
  ...
  idx_dimensao <- !periodo_invalido & !raca_cor_invalido
  if (sum(idx_dimensao) > 0) {
    dados_dimensao <- data.frame(
      Ano = ano[idx_dimensao],
      Trimestre = trimestre[idx_dimensao],
      raca_cor_valor = raca_cor[idx_dimensao],
      valido = as.integer(valido[idx_dimensao]),
      one = 1L,
      stringsAsFactors = FALSE
    )

    total_dimensao <- aggregate(one ~ Ano + Trimestre + raca_cor_valor, data = dados_dimensao, FUN = sum)
    validos_dimensao <- aggregate(valido ~ Ano + Trimestre + raca_cor_valor, data = dados_dimensao, FUN = sum)
    contagem_dimensao <- merge(
      total_dimensao,
      validos_dimensao,
      by = c("Ano", "Trimestre", "raca_cor_valor"),
      all = TRUE
    )
    names(contagem_dimensao) <- c("Ano", "Trimestre", "raca_cor_valor", "registros_total", "registros_validos")
...
if (length(lista_agregados_nivel) == 0) {
  stop("Nao ha registros validos para calcular distribuicao ponderada por raca/cor.")
}
...
total_ponderado_dimensao <- aggregate(
  peso ~ Ano + Trimestre + raca_cor_valor,
  data = agregado_nivel,
  FUN = sum
)
...
names(resultado)[names(resultado) == "raca_cor_valor"] <- nome_campo_dimensao
...
if (length(lista_agregados_contagem) > 0) {
  ...
  contagem_dimensao <- aggregate(
    cbind(registros_total, registros_validos) ~ Ano + Trimestre + raca_cor_valor,
...
names(contagem_dimensao)[names(contagem_dimensao) == "raca_cor_valor"] <- nome_campo_dimensao

resumo_dimensao <- merge(
  contagem_dimensao,
  {
    total_tmp <- total_ponderado_dimensao
    names(total_tmp)[names(total_tmp) == "raca_cor_valor"] <- nome_campo_dimensao
    total_tmp
  },
...
series_por_raca_cor <- lapply(names(split_dimensao), function(valor_dimensao) {
...
json_saida <- list(
  metadata = list(
    ...
    rotulo_dimensao = "Cor ou raca",
    descricao = "Distribuicao percentual ponderada de nivel de instrucao por cor ou raca, ano e trimestre."
  ),
  cortes = cortes_json,
  series_por_raca_cor = series_por_raca_cor,
